...
export const NulidadesPage: React.FC = () => {
  ...
  const fetchNotices = async (q?: string) => {
    ...
    try {
      ...
      const mapped = items.map((item: any) => ({
        id: item.id,
        edital: item.title || item.objeto?.substring(0, 50) || item.id,
        orgao: item.metadata?.orgao || item.orgao || 'Órgão não especificado',
        tipoInfracao: 'ANALISANDO_EDITAL',
        resumoIrregularidade: 'Clique em Analisar Edital para buscar possíveis nulidades.',
        prazoLimiteImpugnacao: item.metadata?.data_abertura || item.data_abertura || 'Indefinido',
        status: 'RASCUNHO'
      }));
      setNulidades(mapped);
    } catch (err) {
  ...
  const handleAnalyze = async (id: string) => {
    ...
    try {
      ...
      const radar = res.data?.data || res.data;
